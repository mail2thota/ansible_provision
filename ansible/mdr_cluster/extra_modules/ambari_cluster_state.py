...
def main():
    ...
    argument_spec = dict(
        ...
        use_local_repository=dict(default=False, required=False, type='bool'),
    )
    ...
    use_local_repository = p.get('use_local_repository')
    ...
    try:
        if cluster_state in ['started', 'stopped']:
            ...
            module.exit_json(changed=True, results=request.content,
                             created_blueprint=created_blueprint, status=request_status, request_id= request_id,req_data=data1, blueprint=blueprint)


    except requests.ConnectionError, e:
        module.fail_json(msg="Could not connect to Ambari client: {0}, {1}".format(str(e.message),configurations))
    except Exception, e:
        module.fail_json(msg="Ambari client exception occurred: {0}, {1}" .format(str(e.message),configurations))
...
def create_cluster(ambari_url, user, password, cluster_name, blueprint_name, configurations, hosts_json,default_password):
    ...
    if r.status_code != 202:
        msg = 'Could not create cluster: request code {0}, \
                    request message {1}, cluster_config {2}'.format(r.status_code, r.content, data)
        raise Exception(msg)
    return r

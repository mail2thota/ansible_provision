...
class Match(object):
    ...
    def __call__(self, v):
        try:
            ...
            raise MatchInvalid(self.msg + ":" +str(v))
        if not match:
            raise MatchInvalid(self.msg + ":" +str(v) or 'does not match regular expression')
        return v
...
def FqdnUrl(v):
    ...
    >>> s = Schema(FqdnUrl())
    >>> with raises(MultipleInvalid, 'expected a Fully qualified domain name URL('+v+')'):
    ...   s("http://localhost/")
    >>> s('http://w3.org')
    'http://w3.org'
    """
...
def Url(v):
    ...
    >>> s = Schema(Url())
    >>> with raises(MultipleInvalid, 'expected a URL('+v+')'):
    ...   s(1)
    >>> s('http://w3.org')
    'http://w3.org'
    """
...
class Componentgroups(object):

    def __init__(self,  msg=None):
        self.msg = msg

    def __call__(self, componentgroups):
        #Check for miniumu one group
        # Validate the compoonetn groups element uniqueness in each group
        if componentgroups == None:
            raise Invalid("hdp Component groups should not be empty must have atleast one component group to be used blueprint")
        for component_group in componentgroups:
            if componentgroups[component_group] == None:
                raise Invalid("hdp component group ["+component_group+"] should not be empty ")
...
class Number(object):
    ...
    def __call__(self, v):
        ...
        if self.precision is not None and self.scale is not None and\
            ...
            raise Invalid(self.msg +str(v) + "(Note: Precision must be equal to %s, and Scale must be equal to %s" %(self.precision, self.scale)+")")
        else:
            if self.precision is not None and precision != self.precision:
                raise Invalid(self.msg+str(v) + "(Note: Precision must be equal to %s"%self.precision+")")

            if self.scale is not None and scale != self.scale :
                raise Invalid(self.msg + "(Note: Scale must be equal to %s"%self.scale+")")

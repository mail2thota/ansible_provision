...
setHostName(){
    ...
    then
        ...
        unset https_proxy
        unset no_proxy
        #adapter/device network only can be assigned by 1 ip
        #provide checking mechanism
        host_ip=$(ip addr show $dhcp_interface | grep -Po 'inet \K[\d.]+')
        hostname=$(echo $dns_id | cut -d"." -f1)
        echo "127.0.0.1   localhost localhost.localdomain localhost4 localhost4.localdomain4" > /etc/hosts
        echo "::1         localhost localhost.localdomain localhost6 localhost6.localdomain6" >> /etc/hosts
        echo "$host_ip $dns_id $hostname" >> /etc/hosts
        netmanager="/etc/NetworkManager/NetworkManager.conf"
        grep -q "dns=none" $netmanager||sed -i '/\[main\]/a dns=none' $netmanager
        systemctl restart NetworkManager 
        echo "#Generated by NetworkManager
search example.com 
nameserver ${host_ip}" > /etc/resolv.conf
    fi
    sudo hostnamectl set-hostname $dns_id
